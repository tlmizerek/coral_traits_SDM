...
ss$winter_par_early <- rowMeans(ss[, grep('9._06_|9._07_|2000_06_|2000_07_', names(s1), val=TRUE)])
...
model.covs <- data.frame(present=model.datasp$present,
                          ...
                          June98=model.datasp$par_1998_06_01, 
                          July98=model.datasp$par_1998_07_01,
                          June99=model.datasp$par_1999_06_01, 
                          July99=model.datasp$par_1999_07_01, 
                          June00=model.datasp$par_2000_06_01, 
                          July00=model.datasp$par_2000_07_01) 


level.plot(model.datasp[,6], model.datasp[,5:4])
level.plot(model.datasp[,7], model.datasp[,5:4])
level.plot(model.datasp[,8], model.datasp[,5:4])
level.plot(model.datasp[,9], model.datasp[,5:4])
level.plot(model.datasp[,10], model.datasp[,5:4])
level.plot(model.datasp[,11], model.datasp[,5:4])
multiple.plot(model.datasp[,6:11], model.datasp[,5:4], cex=0.7)

par(mfrow=c(2,3))
plot(density(na.omit(model.covs$June98)),main="9806PAR")
plot(density(na.omit(model.covs$July98)),main="9807PAR")
plot(density(na.omit(model.covs$June99)),main="9906PAR")
plot(density(na.omit(model.covs$July99)),main="9907PAR")
plot(density(na.omit(model.covs$June00)),main="0006PAR")
plot(density(na.omit(model.covs$July00)),main="0007PAR")

pairs(~SSTraw+TSMraw+Chlraw+ PARraw + skewnessraw +varraw, data=model.covs,
      main="Scatterplot Matrix of raw environmental variables")
log.9806 <- log(model.covs$June98)
log.turb <- log(model.covs$TSMraw)
log.chl <- log(model.covs$Chlraw)
log.skew <- log(model.covs$skewnessraw)
log.var <- log(model.covs$varraw)
log.PAR <- log(model.covs$PARraw)

model.covs$PAR <- ((log.9806-mean(log.9806, na.rm=TRUE))/(sd(log.9806, na.rm=TRUE)))
...
model.covs$skew <- ((log.skew-mean(log.skew, na.rm=TRUE))/(sd(log.skew, na.rm=TRUE)))
...
model.covs$lower <- as.numeric(as.vector(model.datasp$Depth.lower))
model.covs$upper <- as.numeric(as.vector(model.datasp$Depth.upper))

plot(model.covs$cor_size)
plot(model.covs$lower)
...
###PAR (and depth)
modP<- glmer(present ~PAR + (1 +  PAR |species), data= model.covs,family=binomial(link=logit))
modP
coef(modP)
#species distributions of those with negative turb coef should generally be found where less light
plot(coef(modP))

#Species that can be found deeper with larger corallite should be in more turbid water, they're better at expelling sediments and handling the lower light due to turbid water
modP1 <- glmer(present ~ PAR + lower:PAR+(1 +PAR|species), data= model.covs,family=binomial(link=logit))
modP1
coef(modP1)
plot(coef(modP1))


modP2 <- glmer(present ~ PAR + cor_size:PAR+(1 +PAR|species), data= model.covs,family=binomial(link=logit))
modP2
coef(modP2)
plot(coef(modP2))
...
modP3 <- glmer(present ~ PAR + cor_size:PAR+lower:PAR+(1 +PAR|species), data= model.covs,family=binomial(link=logit))
modP3
coef(modP3)

modP4 <- glmer(present ~ PAR + cor_size:PAR+lower:PAR+col_size:PAR+(1 +PAR|species), data= model.covs,family=binomial(link=logit))
modP4
coef(modP4)
...
modT2 <- glmer(present ~ turb + cor_size:turb+col_size:turb+(1 +turb|species), data= model.covs,family=binomial(link=logit))
modT2
coef(modT2)
...
modPt <- glmer(present ~ PAR + turb + cor_size:PAR+lower:PAR+cor_size:turb+lower:turb+(1 +PAR+turb|species), data= model.covs,family=binomial(link=logit))
modPt
coef(modPt)
